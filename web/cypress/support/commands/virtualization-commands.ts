  ...
  waitForVirtualizationReady(KBV: { namespace: string }): void {
    ...
    cy.exec(`oc get csv -n openshift-cnv | grep kubevirt | awk '{print $1}'`)
      ...
      .then((operatorName) => {
        ...
        cy.exec(
          `sleep 15 && oc wait --for=jsonpath='{.status.phase}'=Succeeded ClusterServiceVersion/${KBV_OPERATOR_NAME} -n openshift-cnv --timeout=300s --kubeconfig ${Cypress.env('KUBECONFIG_PATH')}`
          , {
            timeout: 315000 // Set a long timeout for the 'oc wait' command
          }
        );
      });
